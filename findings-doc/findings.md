...
- Replay status: **Fixed; independently verified on 2026-08-24 by deterministic regression and a fresh strict-headless Portuguese 2028 election capture**
- Evidence:
  ...
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-pt435-20260824/focused-pt435-election.log`,
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-pt435-20260824/pt435-election-nominees.json`,
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-pt435-20260824/pt435-election-text.txt`,
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-pt435-20260824/pt435-live-check.txt`,
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-pt435-20260824/focused-pt457-car-theft.log`, and
...
- Independent verification: detached clean clones at `ba0bce08` reran the focused
  election regression (10 tests, all pass) and a separate strict-headless verifier
  reached the live `Eleição Geral Presidencial 2028` screen. The captured candidate
  rows had localized prose, exactly one comma-space, zero doubled separators, zero
  raw alignment shorthand, and maximum width 80. The same verifier passed the focused
  PT-457 car-theft regression. A bounded January stock route from another verifier did
  not reach the November branch; the direct 2028 payload capture closes that reachability gap.
...
- Replay status: **Verification-pending (2026-08-24)**
- Evidence:
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-layout-batch-d-20260824/focused-batch-c.log`,
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-layout-batch-d-20260824/capture-006-web-import-blocker.json`, and
  `lib/saveload/save_load.dart:418-424`
- Independent result: the desktop dialog-title regression passes against build `2d10f02d`, but the
  preserved replay requires a native Windows/Linux picker. The web probe leaves the Portuguese
  title unchanged and exposes no file input, so this remains pending rather than passing.
...
- Independent result: the width-safe malformed-array widget regression passes against build
  `2d10f02d`. The web import action exposes no file input for the required local malformed-file
  replay, so complete live diagnostic wrapping remains pending.
...
- Replay status: **Verification-pending — coverage-only (2026-08-24)**
- Evidence:
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-layout-batch-d-20260824/focused-batch-c.log`,
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-layout-batch-d-20260824/focused-context-map.log`,
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-layout-batch-d-20260824/capture-001-title-pt.json`, and
  `lib/map_editor/map_editor_screen.dart:483-516`
- Independent result: direct `MapEditorScreen` probes pass at 320×240 and 480×320 with zero render
  exceptions. The public title has no Mod Tools entry because the production debug flag is off;
  without explicit authorization to enable that flag, stock/manual narrow-editor coverage remains
  pending.
...
- Evidence:
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-layout-batch-d-20260824/focused-batch-c.log`,
  `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/verify-layout-batch-d-20260824/focused-context-map.log`, and
  `lib/daily/hostages/interrogate.dart:99-111`
- Independent result: masculine/feminine article regressions pass against build `2d10f02d`. A fresh
  ordinary interrogation route did not reach the mapped/no-new-info workplace branch during this
  verification pass, so the required live buffer capture is still missing.
...
- Independent result: quoted-question punctuation regressions pass against build `2d10f02d`. The
  ordinary interrogation branch was not reached in a live buffer during this verification pass;
  keep pending until captured directly.
...
- Independent result: masculine/feminine/nonbinary ransom outcome regressions pass against build
  `2d10f02d`. No female ransom-pickup arrest/narrow-escape branch was reached through ordinary
  play in this pass, so live gendered-output evidence remains pending.
...
- Independent result: masculine/feminine/nonbinary Sleeper and President regressions pass against
  build `2d10f02d`. Fresh female/nonbinary sleeper-President branches were not reached through
  ordinary monthly play here; retain pending until direct captures exist.
...
- Replay status: **Fixed in isolated verifier branch; natural low-level screen replay pending**
- Source snapshot: `5f5e52d37823a922f69abc4158c35e089a8a8df9`
- Exact source: `lib/creature/level.dart:38-73`
- Pre-fix deterministic runtime probe: `/home/henry/tmp/agent-tmp/lcs-new-age-playtest/prober2-20250825-r1/pre-fix-level-title-probe.log`

### Actual

`levelTitle` called `LcsI18n.tr` for four absent keys. In `pt_BR`, every alignment returned raw English for levels -2, -1, and 0: `In Crisis`, `Unstable`, and `Struggling`. Moderate level 6 also returned raw `Peacemaker`. The probe exercised all three alignments and is a runtime call-path confirmation, not a static catalog scan. PT-467 (Eminent Scientist) is separate and is not conflated here.

### Fix and verification status

The isolated fix adds English catalog entries and Portuguese values `Em Crise`, `Instável`, `Em Dificuldades`, and `Pacificador`. The focused level test covers all alignments, negative levels, the moderate peacemaker title, English preservation, and `strLenX` <= `CONSOLE_WIDTH`. The isolated release build passed, and both locale catalogs pass canonical hash-shard validation. A fresh strict-headless smoke route used server `127.0.0.1:14681`, session `prober2-level-fix`, and namespace `lcs-new-age-prober2-level-fix`; it confirmed Portuguese setup/base screens but did not naturally reach a low-level profile, so natural-screen closure remains pending.
