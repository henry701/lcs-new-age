...
- Replay status: **Open; reproduced in strict-headless Portuguese replay on 2026-07-30**

The default founder generated by the Portuguese new-game route was Manuela
Frank, but several biography choices used masculine agreement: `Eu era
travesso`, `Eu era obcecado`, `Fui pego hackeando minhas notas`, and `Fui pego
ficando com outra garota`. Selecting those choices makes the founder narrate
herself with the wrong gender. The source strings are static before the
founder's sex is finalized, so a future fix should use gender-neutral wording
or select a sex-aware variant without breaking the English catalog.
...
and `Cota de mithril`, but the footer still says `Pressione uma letra para
comprar roupas`. The generic source key is reused for clothing and armour;
this screen needs an armour-specific prompt or a neutral purchase instruction.
...
The two status actions are drawn into the same 80-column row. Portuguese text
overwrites the first label and clips the second, producing
`0 - Mostrar o status Liberal do esquadr# - Verificar o status de um Liberal do`.
The action row needs wrapping, a narrower label, or a responsive layout mode
when the console is displayed in a narrow viewport.
